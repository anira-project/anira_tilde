...
size_t element_count(const std::vector<int64_t>& shape) {
    size_t n = 1;
    for (int64_t const d : shape) { n *= static_cast<size_t>(d); }
    return n;
}
...
std::vector<size_t> message_subchannels(size_t total_elems, size_t declared_channels) {
    size_t const channels = declared_channels == 0 ? 1 : declared_channels;
    ...
    std::vector<size_t> subchannels(channels, elems_per_ch);
    return subchannels;
}
...
bool tensor_referenced(const std::vector<StatePair>& pairs,
    ...
    for (const auto& p : pairs) {
        if ((side_is_input ? p.m_input_tensor : p.m_output_tensor) == tensor_index) { return true; }
    }
    return false;
}
...
TensorLayout TensorLayout::from(const anira::InferenceConfig& cfg,
    ...
    TensorLayout layout;
    layout.m_state_pairs = std::move(state_pairs);
    layout.m_input_block_sizes = cfg.m_processing_spec.m_preprocess_input_size;
    layout.m_output_block_sizes = cfg.m_processing_spec.m_postprocess_output_size;
    ...
    for (size_t i = 0; i < in_shape.size(); ++i) {
        if (layout.m_input_block_sizes[i] > 0) {
            layout.m_sig_input_channels.push_back(in_channels[i]);
        } else if (tensor_referenced(layout.m_state_pairs, i, /*side_is_input=*/true)) {
            // State input tensor: managed internally, no Max inlet.
        } else {
            layout.m_msg_input_channels.push_back(
                message_subchannels(element_count(in_shape[i]), in_channels[i]));
        }
    }

    for (size_t i = 0; i < out_shape.size(); ++i) {
        if (layout.m_output_block_sizes[i] > 0) {
            layout.m_sig_output_channels.push_back(out_channels[i]);
        } else if (tensor_referenced(layout.m_state_pairs, i, /*side_is_input=*/false)) {
            // State output tensor: fed back internally, no Max outlet.
        } else {
            layout.m_msg_output_channels.push_back(
...
void TensorLayout::build_channel_maps() {
    m_input_channel_to_tensor.clear();
    m_input_channel_in_tensor.clear();
    for (size_t t = 0; t < m_sig_input_channels.size(); ++t) {
        for (size_t c = 0; c < m_sig_input_channels[t]; ++c) {
            m_input_channel_to_tensor.push_back(t);
            m_input_channel_in_tensor.push_back(c);
        }
    }
    m_output_channel_to_tensor.clear();
    m_output_channel_in_tensor.clear();
    for (size_t t = 0; t < m_sig_output_channels.size(); ++t) {
        for (size_t c = 0; c < m_sig_output_channels[t]; ++c) {
            m_output_channel_to_tensor.push_back(t);
            m_output_channel_in_tensor.push_back(c);
        }
    }
}
