...
ResamplerConfig parse_resampler_config(std::istream& stream) {
    ...
    try {
        ...
        if (rs.contains("model_sample_rate")) {
            result.model_sample_rate = rs.at("model_sample_rate").get<double>();
        }
        if (rs.contains("quality")) {
            result.quality =
                quality_from_string(rs.at("quality").get<std::string>(), result.quality);
        }
        parse_quality_map(rs, "input_quality", result.quality, result.input_quality);
        parse_quality_map(rs, "output_quality", result.quality, result.output_quality);
    } catch (...) {}
    return result;
}
