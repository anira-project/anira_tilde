...
#include "c74_min_object_wrapper.h"
    ...
    , m_maxclass_setup(
          ...
          MIN_FUNCTION {  // NOLINT(misc-unused-parameters) macro-fixed signature
              ...
              c74::max::class_addmethod(c,
                                        reinterpret_cast<c74::max::method>(mc_multichanneloutputs),
                                        "multichanneloutputs",
                                        c74::max::A_CANT,
                                        0);
              c74::max::class_addmethod(c,
                                        reinterpret_cast<c74::max::method>(mc_inputchanged),
                                        "inputchanged",
                                        c74::max::A_CANT,
                                        0);
              return {};
...
void McAniraTilde::create_signal_ports(const std::vector<size_t>& sig_inputs,
    ...
    for (size_t t = 0; t < sig_inputs.size(); ++t) {
        Input in;
        in.m_inlet =
            std::make_unique<c74::min::inlet<>>(this, tensor_label("mc signal", t), "signal");
        in.m_type = MaxType::SIGNAL;
        in.m_num_channels = sig_inputs[t];
        in.m_tensor_index = t;
    ...
    for (size_t t = 0; t < sig_outputs.size(); ++t) {
        Output out;
        out.m_outlet = std::make_unique<c74::min::outlet<>>(this,
                                                            tensor_label("mc signal", t),
                                                            "multichannelsignal");
        out.m_type = MaxType::SIGNAL;
        out.m_num_channels = sig_outputs[t];
...
long McAniraTilde::mc_multichanneloutputs(c74::max::t_object* x, long index) {
    ...
    if (index >= 0 && static_cast<size_t>(index) < self.m_sig_outlets.size()) {
        return static_cast<long>(self.m_sig_outlets[index].m_num_channels);
    }
    return 0;
}
...
long McAniraTilde::mc_inputchanged(c74::max::t_object* x, long index, long count) {
    ...
    if (index >= 0 && static_cast<size_t>(index) < self.m_sig_inlets.size()) {
        const size_t expected = self.m_sig_inlets[index].m_num_channels;
        if (count > 0 && std::cmp_not_equal(count, expected)) {
            c74::max::object_warn(
                x,
